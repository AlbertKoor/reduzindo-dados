...
function totalProducts(array) {
  return array.reduce((acumulador, produto) => {
    return acumulador + (produto.price || 0);
  }, 0);
}
...
function totalProductsSize(array) {
  return array
    .filter((produto) => produto.size === "GG")
    .reduce((acumulador, produto) => acumulador + produto.price, 0);
}
...
function totalProductsSale(array) {
  return array
  .filter((produto) => produto.sale == true)
  .reduce((acumulador, produto) => acumulador + (produto.price * 0.5 || 0),0)
}
